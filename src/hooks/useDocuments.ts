...
export const useDocuments = (filters?: DocumentFilters) => {
  ...
  // 4. Update Document (Admin only) — PUT /documents?documentUrl=<url>
  const update = useMutation({
    mutationFn: async ({
      ...
      const { data } = await api.put<any>("/documents", documentData, {
        params: { documentUrl },
      });
      return data.data?.document || data.document || data.data || data;
    },
  ...
  // 5. Delete Document (Admin only) — DELETE /documents?documentUrl=<url>
  const remove = useMutation({
    mutationFn: async (documentUrl: string) => {
      const { data } = await api.delete<any>("/documents/upload", {
        params: { documentUrl },
      });
      return data;
    },
  ...
  // 6. Download Document via GET /documents/download?url=...
