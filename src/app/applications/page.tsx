...
function StudentSidebar({ counts }: { counts: Record<string, number> }) {
  ...
  const STUDENT_MENU = [
    ...
    {
      href: "/interviews",
      label: t.applications.interviews,
      icon: CalendarClock,
    },
    {
      href: "/scholarships",
      label: t.applications.browseScholarships,
      icon: GraduationCap,
    },
    ...
    <aside className="w-64 shrink-0 hidden lg:block">
      <Card className="sticky top-24 border-none shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    {href === "/applications" && counts.all > 0 && (
                      <Badge
                        className={cn(
                          "h-5 min-w-[20px] text-[10px] px-1.5 rounded-full",
        ...
        <div className="border-t mx-2 mt-1 pt-3 pb-2 px-2">
          <p className="text-[10px] font-bold tracking-widest text-muted-foreground mb-2 px-1">
            {t.applications.quickStats}
          </p>
...
export default function ApplicationsPage() {
  ...
  const STATUS_TABS = [
    ...
    {
      value: "UNDER_REVIEW" as TabValue,
      label: t.applications.underReview,
      icon: AlertCircle,
      color: "text-blue-500",
    },
    {
      value: "ACCEPTED" as TabValue,
      label: t.applications.accepted,
      icon: CheckCircle2,
      color: "text-emerald-500",
    },
    {
      value: "REJECTED" as TabValue,
  ...
  const applications = Array.isArray(myApplications.data)
    ? myApplications.data
    : [];
...
function ApplicationCard({ application }: { application: any }) {
  ...
  const STATUS_CONFIG: Record<
    string,
    { label: string; icon: any; badge: any; border: string }
  > = {
    PENDING: {
      label: t.applications.pending,
      icon: Clock,
  ...
  return (
    <Card
      className={cn(
        "border-l-4 hover:shadow-md transition-all group",
        cfg.border,
      )}
    >
